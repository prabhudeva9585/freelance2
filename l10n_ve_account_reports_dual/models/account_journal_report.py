...
class JournalReportCustomHandler(models.AbstractModel):
    ...
    def _get_custom_display_config(self):
        res = super(JournalReportCustomHandler,self)._get_custom_display_config()
        if res.get('components'):
            res.get('components').update({'AccountReportFilters': 'l10n_ve_account_reports_dual.search_template_custom'})
        else:
            res['components'] = {'AccountReportFilters': 'l10n_ve_account_reports_dual.search_template_custom'}
        return res
                ...
                if column.get('expression_label') not in ['additional_col_1', 'additional_col_2']:
                    if column.get('expression_label') == 'account':
                        ...
                        if values['journal_type'] == 'sale':
                            col_value = values['date'] if values['debit'] else ''
                        elif values['journal_type'] == 'purchase':
                            col_value = values['date'] if values['credit'] else ''
    ...
    def _get_generic_tax_summary_for_sections(self, options, data):
        ...
        print(">>>>>>>>>>>>>>>> tax_report_options >>>>>>>>>",tax_report_options)
        ...
        for tax in taxes:
            ...
            res[tax.country_id.name].append({
                'base_amount': report.format_value(options,tax_values[tax.id]['base_amount'], blank_if_zero=False, currency=self.env['res.currency'].browse(options.get('selected_currency')), figure_type='monetary'),
                'tax_amount': report.format_value(options,tax_values[tax.id]['tax_amount'], blank_if_zero=False, currency=self.env['res.currency'].browse(options.get('selected_currency')), figure_type='monetary'),
                'name': tax.name,
