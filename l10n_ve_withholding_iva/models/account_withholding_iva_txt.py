...
class WithholdingIVATXT(models.Model):
	...
	name = fields.Char('Descripción', required=True, readonly=True, 
                    # states={'draft': [('readonly', False)]} moved to xml
                    )
	start_date = fields.Date(string='Desde', required=True, readonly=True, 
                        #   states={'draft': [('readonly', False)]} moved to xml
	)
	end_date = fields.Date(string='Hasta', required=True, readonly=True, 
                        # states={'draft': [('readonly', False)]} moved to xml
                        )
	...
	line_ids = fields.One2many('account.withholding.iva', 'txt_id', string='Líneas', readonly=True, 
                            # states={'draft': [('readonly', False)]} moved to xml
                        )
